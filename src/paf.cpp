...
// ---- Fast mean ----
inline double fast_mean(const NumericVector &x) {
  int n = x.size();
...
SEXP paf_cpp(NumericVector y, NumericVector a) {
  ...
  // normalize y
  double mean_y = fast_mean(y);
  for (int i = 0; i < n; i++) y[i] /= mean_y;
  
  double sd_y = fast_sd(y);
...
SEXP paf_parallel(NumericVector y, NumericVector a, int ncores = 1) {
  ...
  double mean_y = parallel_mean(y, ncores);
  for (int i = 0; i < n; i++) y[i] /= mean_y;
  
  // sd of normalized y has mean 1.0
  double sd_y = parallel_sd(y, 1.0, ncores);
  double com  = 4.7 / std::sqrt(double_n) * sd_y;
  
  // alienation (independent of alpha), tight loop
  double alien = 0.0;
  const double * RESTRICT yptr = y.begin();
  for (int i = 0; i < n; i++) {
    double yi = yptr[i];
  ...
  for (int k = 0; k < lena; k++) {
    ...
    KernelPAFWorker worker(y, alpha, inv_h_sq, norm_const, inv_n);
    tbb::global_control gc(tbb::global_control::max_allowed_parallelism, ncores);
...
// [[Rcpp::export]]
SEXP paf(NumericVector y, NumericVector a, int ncores = 1) {
